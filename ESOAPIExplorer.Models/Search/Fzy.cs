...
public class Fzy : ISearchAlgorithm
{
        ...
        for (int i = 0; i < haystack.Length; i++)
        {
            ...
            switch (true)
            {
                case true when lastChar == '/' || lastChar == '\\':
                    matchBonus[i] = Scoring.Match.Slash;
                    break;
                case true when lastChar == '-' || lastChar == '_' || lastChar == ' ':
                    matchBonus[i] = Scoring.Match.Word;
                    break;
                case true when lastChar == '.':
                    matchBonus[i] = Scoring.Match.Dot;
                    break;
                case true when char.IsLower(lastChar) && char.IsUpper(thisChar):
                    matchBonus[i] = Scoring.Match.Capital;
                    break;
                default:
                    matchBonus[i] = 0;
                    break;
    ...
    private static void Compute(string needle, string haystack, double[][] D, double[][] M, bool caseSensitive)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            double prevScore = Scoring.Min;
            double gapScore = i == n - 1 ? Scoring.Gap.Trailing : Scoring.Gap.Inner;
            ...
            for (int j = 0; j < m; j++)
            {
                ...
                {
                    double score = Scoring.Min;

                    if (i == 0)
                    {
                        score = (j * Scoring.Gap.Leading) + matchBonus[j];
                    }
                    else if (j > 0)
                    {
                        double a = M[i - 1][j - 1] + matchBonus[j];
                        double b = D[i - 1][j - 1] + Scoring.Match.Consecutive;
                        score = Math.Max(a, b);
                    }
    ...
    private static double Score(string needle, string haystack, bool caseSensitive = false)
    {
        ...
        if (n == 0 || m == 0 || m > Scoring.MaxMatchLength || n > m)
        {
            return Scoring.Min;
        }
        else if (n == m)
    ...
    private static PositionResult Positions(string needle, string haystack, bool caseSensitive = false)
    {
        ...
        if (n == 0 || m == 0 || m > Scoring.MaxMatchLength || n > m)
        {
            return new PositionResult([], Scoring.Min);
        }
        else if (n == m)
        {
            int[] consecutive = Enumerable.Range(1, n).ToArray();

            return new PositionResult(consecutive, Scoring.Max);
        }
        ...
        for (int i = n - 1; i >= 0; i--)
        {
            while (j >= 0)
            {
                if (D[i][j] != Scoring.Min && (matchRequired || D[i][j] == M[i][j]))
                {
                    matchRequired = (i != 0) && (j != 0) && (M[i][j] == D[i - 1][j - 1] + Scoring.Match.Consecutive);
                    positions[i] = j + 1;
                    j--;
                    break;
                }
